...
if page == "App Browser":
    ...
    st.markdown("Search and filter apps by demographic profile")

    col1, col2, col3, col4 = st.columns(4)
    gender_filter = col1.selectbox(
        "Gender", ["All", "female", "male", "neutral"]
    )
    tier_filter = col2.selectbox(
        "Signal Tier", ["All", "S", "A", "B", "C"]
    )
    category_filter = col3.selectbox(
        "Category",
        ["All", "Shopping", "Finance", "Health & Fitness",
         "Food & Drink", "Social", "Communication"]
    )
    ...
    st.dataframe(
        df[['app_name', 'category', 'gender_label',
            'signal_tier', 'age_primary', 'income_label',
            'rating', 'installs']].rename(columns={
            'app_name': 'App Name',
            'category': 'Category',
            'gender_label': 'Gender',
            'signal_tier': 'Tier',
            'age_primary': 'Age Group',
            'income_label': 'Income',
            'rating': 'Rating',
            'installs': 'Installs'
        }),
...
# =====================
    ...
    selected = st.selectbox("Select an app", app_names)
    ...
    if selected:
        ...
        with col2:
            ...
            tier = app['signal_tier']
            st.write(f"**Signal Tier:** {tier_colors.get(tier, '')} {tier}")
        ...
        new_gender = col3.selectbox(
            "New Gender", ["female", "male", "neutral"]
        )
        new_tier = col4.selectbox("New Tier", ["S", "A", "B", "C"])
        if col5.button("Save Override"):
            response = requests.post(
                f"{API_URL}/api/v1/apps/{app['package_name']}/override",
                params={
                    "gender": new_gender,
                    "signal_tier": new_tier
                }
            )
...
# =====================
    ...
    st.markdown("Low confidence apps that need manual review")
