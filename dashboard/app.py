...
def get_all_apps():
    ...
    try:
        ...
        st.error("Failed to load apps from database")
        return pd.DataFrame()
    finally:
        conn.close()
...
def get_stats():
    ...
    try:
        ...
        cursor.execute("SELECT SUM(tokens_used) FROM app_classifications WHERE country='in'")
...
if page == "App Browser":
    ...
    if not df.empty:
        ...
        st.dataframe(
            df[['app_name','category','gender_label','signal_tier',
                'age_primary','income_label','rating','installs']].rename(columns={
                'app_name':'App Name',
                'category':'Category',
                'gender_label':'Gender',
                'signal_tier':'Tier',
    ...
    if stats:
        ...
        col4.metric("Total Tokens Used", f"{stats['total_tokens']:,}")
    ...
    st.markdown("Apps with medium confidence or weak signal — need manual verification")
    ...
    try:
        df = pd.read_sql("""
            ...
            WHERE m.country='in'
            AND (
                c.gender_confidence='medium'
                OR c.signal_tier='B'
                OR c.gender_score < 0.65
            )
            ORDER BY m.rating_count DESC NULLS LAST
            LIMIT 20
        """, conn)
        ...
        if len(df) == 0:
            ...
            st.dataframe(
                df[['app_name','package_name','installs',
                    'gender_label','signal_tier',
                    'gender_confidence','gender_score']].rename(columns={
                    'app_name':'App Name',
